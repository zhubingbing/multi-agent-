...
interface Props {
  ...
  onAudioUnlock?: () => void;
  draftKey?: string;
  /** Human and Agent participants shown before files in the @ autocomplete menu. */
  mentions?: MentionOption[];
  /** Session working directory — enables file results in the @ autocomplete menu. */
  cwd?: string | null;
}
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  // ── @ participant and file autocomplete ─────────────────────────────────
  // Recomputed from the text before the caret on every change/caret move.
  // Participants remain available without a cwd; file results require one.
  const updateAtQuery = useCallback((text: string, cursor: number | null) => {
    if (!cwd && mentions.length === 0) {
      setAtQuery(null);
      return;
    }
  ...
  const atMentionMatches: FileIndexEntry[] = React.useMemo(() => {
    if (atQueryText === null) return [];
    const query = atQueryText.toLocaleLowerCase();
    return mentions
      .filter((mention) => !query || mention.id.toLocaleLowerCase().includes(query) || mention.label.toLocaleLowerCase().includes(query))
      .map((mention) => ({ path: `mention:${mention.id}`, isDir: false }));
  }, [atQueryText, mentions]);
  const atFileMatches: FileIndexEntry[] = serverResultInUse ? atServerResult.matches : atLocalMatches;
  const atMatches: FileIndexEntry[] = [...atMentionMatches, ...atFileMatches].slice(0, 20);
  ...
  const applyAtCompletion = useCallback((entry: FileIndexEntry) => {
    ...
    const mentionID = entry.path.startsWith("mention:") ? entry.path.slice("mention:".length) : null;
    const insert = mentionID
      ? { text: `@${mentionID} `, cursorOffset: mentionID.length + 2 }
                ...
                >
                  <span>
                    {indexLoading
                      ? t("chat.loadingFiles")
                      : mentions.length > 0
                        ? `提及成员或文件 · ${atMatches.length} 个匹配项${truncatedHint ? ` · ${truncatedHint}` : ""}`
                ...
                <div style={{ maxHeight: "calc(min(48vh, 400px) - 34px)", overflowY: "auto", padding: 4 }}>
                  {!indexLoading && atMatches.length === 0 ? (
                    ...
                    atMatches.map((entry, index) => {
                      ...
                      const mentionID = entry.path.startsWith("mention:") ? entry.path.slice("mention:".length) : null;
                      const mention = mentionID ? mentions.find((candidate) => candidate.id === mentionID) : undefined;
                      const name = mention?.label ?? entry.path.split("/").pop() ?? entry.path;
                      const dirPrefix = mention ? "" : entry.path.slice(0, entry.path.length - name.length);
                      return (
                        <button
                          key={mention ? `mention:${mention.id}` : `${entry.isDir ? "d" : "f"}:${entry.path}`}
                          ref={(node) => {
                        ...
                        >
                          <span style={{ flexShrink: 0, display: "flex", alignItems: "center" }}>
                            {mention
                              ? <span style={{ display: "grid", width: 22, height: 22, placeItems: "center", borderRadius: 6, background: "var(--bg-selected)", color: "var(--accent)", fontSize: 10, fontWeight: 700 }}>{mention.label.slice(0, 1).toUpperCase()}</span>
                              : entry.isDir ? <FolderIcon size={14} /> : getFileIcon(name, 14)}
                          </span>
                          <span style={{ minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {mention ? <>
                              <span>{mention.label}</span>
                              <span style={{ marginLeft: 8, color: "var(--text-dim)", fontSize: 10 }}>{mention.kind === "agent" ? "AGENT" : "HUMAN"}{mention.description ? ` · ${mention.description}` : ""}</span>
                            </> : <>
